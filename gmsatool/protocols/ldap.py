from ldap3 import ALL, ALL_ATTRIBUTES, ENCRYPT, KERBEROS, MODIFY_REPLACE, NTLM, SASL, SCHEMA, TLS_CHANNEL_BINDING, Connection, Server
...
def get_ldap_session(domain, dc, ldaps, username, password, kerberos=False, all_info=False):
    ...
    if kerberos is False:
        ...
        if ldaps is True:
            ...
            ldap_session = Connection(
                server, authentication=SASL, sasl_mechanism=KERBEROS, auto_bind=True, raise_exceptions=True, session_security=ENCRYPT
            )
    return ldap_session
...
def get_entry(ldap_session, dn, search_filter="(objectClass=*)", attributes=ALL_ATTRIBUTES, get_operational_attributes=False, controls=None):
    ...
    ldap_session.search(
        search_base=dn, search_filter=search_filter, attributes=attributes, get_operational_attributes=get_operational_attributes, controls=controls
    )
...
def sid_to_samaccountname(ldap_session, dn, sid):
    ...
    if sid_str in well_known_sids_sid_name_map:
        name = well_known_sids_sid_name_map[sid_str]
...
def check_group_membership(ldap_session, dn, username, group_dn):
    ...
    ldap_session.search(
        search_base=dn,
        search_filter=f"(&(sAMAccountName={username})(memberOf:1.2.840.113556.1.4.1941:={group_dn}))",
